...
Gibbs tracker) frame-by-frame, sequentially — so frame 0 IS frame 0 (the
threaded live demo loops during its slow init, which is why its frame-0
segmentation was wrong). Frame 0 is SEEDED from the **ground-truth** segmask for
...
# flat 2D image; 1.5° read as nearly-flat (object barely separable from the
# background slab), so 8° was chosen after a visual sweep (1.5 vs 12) as the best
# depth-vs-frontal tradeoff for the demo. Override with PC_YAW.
...
def _stats_row(width, st):
    ...
    # Outlier fraction: white at ~0 (the v2 target), amber if it creeps up.
...
def render_video(vid: str, out_path: Path, *, yaml_cfg: dict, num_sweeps: int,
    ...
    # ONE global value — no per-video knobs (calibrated/selected on the TRAIN split).
    ...
    # which DEFAULTS them to the vendored False. Read from YAML + thread through
    # or the v2 fix (frozen semantic clusters) never reaches the tracker.
    ...
    # CONVERGE anti-bleed lever: per-frame gibbs_blob_means refinement count
    ...
    for i, bgr, is_seam in _iter_frames_looped(Path(src), max_frames, target_frames, frame_stride):
        ...
        # so without a re-seed later loops look progressively worse than the
        # first.  We RESTORE the cached post-init frame-0 state (`seed_state`,
        # immutable JAX pytree) rather than re-running init_state -> the CHM
        # membership + blob geometry snap back to the pristine frame-0 semantic
        # seed INSTANTLY (no per-seam K-means/warmup latency hitch), so every loop
        # is as fresh as the first AND the demo stays real-time.  The seam frame
        # then runs the same per-frame step on the restored seed, exactly like the
        # original frame 0.  prev_tensor=None keeps the flow-reset at the seam.
        if is_seam:
            prev_tensor = None
        ...
        # The GT/SAM-propagation hack is REMOVED: with freeze_hyperblob_assignment
        # the frame-0 semantic seed is now sticky (hyperblob region-J 0.51-0.57,
        # outliers ~0), so the raw hyperblobs ARE the good clusters. The frame-0
        # GT/SAM seed still enters at init_state (sam_segmentation=seed_grid) —
        # not here. Hyperblob ids are constant across frames under freeze, so
        # render_matter_tile's palette[id] lookup is stable (no per-frame
        # re-derivation / re-sorting), which kills the cluster-color flicker.
        ...
        pc_cluster, pc_particle, f_used, _proj = genmatter_rt.render_pointcloud_tiles_pair(
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # SAM for local); force it on even if a pre-v2 config is passed.
