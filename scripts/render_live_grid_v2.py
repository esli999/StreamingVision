...
A bigger sibling of ``render_live_grid.py`` (the "classic" 1x5, which is left
BYTE-FOR-BYTE intact and still renders). Same real live inferencer
...
(``feature`` | ``motion`` | ``rgb`` | ``frozen_rgb``). Defaults: col3=feature,
col4=motion. The SAME per-particle coloring drives both the dense-pixel tile
(row1) and the 3-D point cloud (row2), so the clearer representation is obvious
side-by-side.
...
def _color_cluster_tile(hyper_full, outlier_full, num_fg):
    ...
    pal = (genmatter_viz._cluster_palette_for(num_fg, nhyp) if num_fg and num_fg > 0
           else genmatter_viz.HYPERBLOB_PALETTE)
    hb = pal[hyper_full].astype(np.uint8)
    if outlier_full.any():
        hb[outlier_full] = genmatter_viz.OUTLIER_BGR
    return hb
...
def _color_particle_tile(mode, blob_full, outlier_full, *, feat_lut, motion_lut,
    ...
    if mode in ("feature", "motion", "frozen_rgb"):
        ...
        bb = genmatter_viz._avg_rgb_by_label(blob_full, rgb_guide, h, w)
    else:
        raise ValueError(f"unknown coloring mode {mode!r} (choose from {COLOR_MODES})")
    if outlier_full.any():
        bb[outlier_full] = genmatter_viz.OUTLIER_BGR
    return bb
...
def render_video(vid: str, out_path: Path, *, yaml_cfg: dict, num_sweeps: int,
    ...
    # Threaded from YAML so the demo tracker behaves exactly like the shipped
    ...
    for i, bgr, is_seam in _iter_frames_looped(Path(src), max_frames, target_frames, frame_stride):
        ...
        if frozen_rgb_lut is None:
            frozen_rgb_lut = genmatter_viz.compute_blob_color_lut(
                blob_a, indices, bgr, h=H, w=W, stride=stride, num_blobs=nb)
        feat_lut = None
        if need_feature:
            feat_lut, feat_basis = genmatter_viz.compute_blob_feature_lut(
                np.asarray(state.blobs_state.blob_features), basis=feat_basis)
        motion_lut = None
        if need_motion:
            motion_lut, fref = genmatter_viz.compute_blob_motion_lut(
        ...
        blob_full, hyper_full, outlier_full = genmatter_viz.render_matter_label_grids(
            blob_a, cluster_lab, indices, h=H, w=W, stride=stride, rgb_guide=bgr, num_blobs=nb)
        ...
        pc_rgb, pc_cluster, f_used, _proj = genmatter_viz.render_pointcloud_tiles_pair(
        ...
        if pc_focal is None and f_used > 0.0:
            pc_focal = f_used
        pc_col3, pc_col4, _f2, _p2 = genmatter_viz.render_pointcloud_tiles_pair(
